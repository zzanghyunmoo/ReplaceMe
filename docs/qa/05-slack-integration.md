...
Slack App이 로컬 API에 접근하려면 ngrok, Cloudflare Tunnel 같은 public tunnel이
필요합니다. 이 프로젝트는 아직 API 인증/인가가 없으므로 tunnel을 열면 Slack endpoint뿐
아니라 전체 local API가 외부에서 접근 가능해질 수 있습니다.
...
- 가능하면 tunnel/proxy에서 `/api/slack/interactivity`만 노출합니다.
...
티켓을 생성하거나 approval을 만들었을 때 기대 결과:

- API가 Slack 설정 누락 때문에 실패하지 않습니다.
- approval request의 Slack message reference는 `not-configured` 계열일 수 있습니다.
