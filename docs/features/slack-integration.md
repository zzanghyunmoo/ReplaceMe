...
Slack App interactivity request URL은 `/api/slack/interactivity` **한 경로만**
upstream으로 전달하는 path-restricted proxy/tunnel에 연결합니다. 전체 8080 port를
공개하는 tunnel은 사용하지 않습니다. Path allowlist를 보장할 수 없으면 실제 Slack
callback QA를 실행하지 않습니다.
...
- `POST /api/slack/interactivity`는 Slack 서명 payload가 필요하므로 단순 curl로는
  성공 케이스를 재현하기 어렵습니다.
- 로컬 callback 테스트는 `/api/slack/interactivity` 외 path를 404/403으로 차단한
  URL에서만 실행합니다.
