...
| Stop conditions | Stop before implementing automation hooks that require the API/worker split, retry/DLQ, Langfuse integration, or full Run Passport persistence. |
...
The infrastructure roadmap in [`2026-07-13-001-feat-infra-foundation-roadmap-plan.md`](./2026-07-13-001-feat-infra-foundation-roadmap-plan.md) is dependency context only. This plan does not duplicate API/worker split, Kafka retry/DLQ, OpenTelemetry Collector, or Langfuse implementation details; it only states which Notion evidence slots those later capabilities may fill.
...
- R11. Until the API/worker split from the infra roadmap is available, lifecycle page creation should remain explicit/manual or endpoint-driven; no background worker hook is required by this plan.
...
**Dependencies:** Run Passport v0 from [`../features/run-passport.md`](../features/run-passport.md). Can be implemented before worker split if exposed only through explicit/manual document creation.
...
### U4. Add idempotent backlink publishing after runtime split is available

**Goal:** Publish lifecycle-page backlinks to Linear and GitHub without duplicate comments or unsafe background coupling.

**Dependencies:** U2, U3, and the API/worker split from [`2026-07-13-001-feat-infra-foundation-roadmap-plan.md`](./2026-07-13-001-feat-infra-foundation-roadmap-plan.md) U1. Retry/DLQ from that roadmap is recommended before automatic background publishing.
...
4. U4 should wait for the API/worker split and preferably retry/DLQ work from the infra roadmap.
...
- **Risk:** Background hooks couple to the current combined API/worker host. **Mitigation:** keep automation hooks gated behind the infra roadmap runtime split.
