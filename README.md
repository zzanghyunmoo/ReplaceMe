...
- `src/DevAutomation.Infrastructure` — EF Core/PostgreSQL, Kafka queue,
  Docker.DotNet agent runner, notifier/repository provider 구현
...
```bash
cp .env.example .env
# .env에 Anthropic/GitHub 또는 GitLab, notifier 값을 입력
...
- Anthropic, GitHub, GitLab, Slack 관련 secret은 로그 저장 전 redaction 대상입니다.
