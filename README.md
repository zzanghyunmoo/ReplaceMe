...
Kafka 큐를 통해 Docker 컨테이너에서 Claude Code를 실행하고, 민감 작업은 MCP
approval tool을 통해 Slack 승인을 받은 뒤 계속 진행합니다.
...
- `src/DevAutomation.Infrastructure` — EF Core/PostgreSQL, Kafka queue,
  Docker.DotNet agent runner, Slack 연동 구현
...
```bash
cp .env.example .env
# .env에 Anthropic/GitHub/Slack 값을 입력

docker compose --profile build-only build agent-image
docker compose up --build api postgres kafka
```

API는 `http://localhost:8080`에서 실행됩니다. 에이전트 컨테이너는 같은 Docker
네트워크(`devautomation-network`)에 붙어 승인 MCP 서버가 PostgreSQL과 Slack
설정을 사용할 수 있게 구성됩니다.
...
- `DEVAUTOMATION_Agent__DockerNetwork`
...
- Anthropic, GitHub, Slack 관련 secret은 로그 저장 전 redaction 대상입니다.
